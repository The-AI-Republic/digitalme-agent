...
export class SessionState {
  ...
  /**
   * Additive mutation — always safe regardless of revision.
   * Used by session memory extraction.
   */
  addMemories(memories: ExtractedMemory[]): void {
    // Append-only — no revision check needed
...
|-----------|---------------|-----------|
| `addMemories()` | None — always apply | Additive, append-only. Stale memories are still valid. |
